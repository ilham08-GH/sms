...
$('#ctrl-volume_spk').on('input', function(){ 
var ctrlVal = $('#ctrl-volume_spk').val();
	var ctrlVal2 = $('#ctrl-harga_satuan').val();
	var ctrlVal3 = ctrlVal*ctrlVal2;
	$('#ctrl-total').val(ctrlVal3);
	var ctrlVal4 = ctrlVal
	$('#ctrl-volume_bast').val(ctrlVal4);
});
$('#ctrl-satuan').on('click', function(){ 
var ctrlVal = $('#ctrl-volume_spk').val();
	var ctrlVal2 = $('#ctrl-harga_satuan').val();
	var ctrlVal3 = ctrlVal*ctrlVal2;
	$('#ctrl-total').val(ctrlVal3);
});
$('#ctrl-harga_satuan').on('change', function(){ 
var ctrlVal = $('#ctrl-volume_spk').val();
	var ctrlVal2 = $('#ctrl-harga_satuan').val();
	var ctrlVal3 = ctrlVal*ctrlVal2;
	$('#ctrl-total').val(ctrlVal3);
});
$('#ctrl-volume').on('input', function(){ 
    var ctrlVal = $('#ctrl-volume').val();
	var ctrlVal2 = $('#ctrl-harga_satuan').val();
	var ctrlVal3 = ctrlVal*ctrlVal2;
	$('#ctrl-total').val(ctrlVal3);
});
$('#ctrl-harga_satuan').on('input', function(){ 
    var ctrlVal = $('#ctrl-volume').val();
	var ctrlVal2 = $('#ctrl-harga_satuan').val();
	var ctrlVal3 = ctrlVal*ctrlVal2;
	$('#ctrl-total').val(ctrlVal3);
...
$(document).ready(function() {
    ...
    let select = document.querySelector('#ctrl-tahun');
    if(select){
        select.innerHTML = "";
        // Tambahkan placeholder
        select.append(new Option('-- Pilih Data --', ''));
        // Tambahkan tahun
        select.append(new Option(year, year));
        select.append(new Option(year - 1, year - 1));
    }
});
$(document).ready(function() {
    // Target semua <select> yang bukan multiple select
    $('select:not([multiple])').each(function() {
        $(this).select2({
            width: '100%',          // full width
            placeholder: '--Pilih Data--',   // teks placeholder
            allowClear: true        // bisa clear pilihan
        });
    });
    // Jika ada <select multiple>
