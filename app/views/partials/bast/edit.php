...
<section class="page" id="<?php echo $page_element_id; ?>" data-page-type="edit"  data-display-type="" data-page-url="<?php print_link($current_page); ?>">
    ...
    <div  class="bg-light p-3 mb-3">
        <div class="container">
            <div class="row ">
                <div class="col ">
                    <h4 class="record-title">Edit  Bast</h4>
                </div>
            </div>
        </div>
    </div>
            ...
            <div class="row ">
                <div class="col-md-7 comp-grid">
                    ...
                    <div  class="bg-light p-3 animated fadeIn page-content">
                        <form novalidate  id="" role="form" enctype="multipart/form-data"  class="form page-form form-horizontal needs-validation" action="<?php print_link("bast/edit/$page_id/?csrf_token=$csrf_token"); ?>" method="post">
                            <div>
                                            ...
                                            <div class="col-sm-8">
                                                <div class="">
                                                    <select required=""  id="ctrl-id_petugas_bast" data-load-path="<?php print_link('api/json/bast_id_petugas_bast_option_list') ?>" data-load-select-options="rincian_output_bast" name="id_petugas_bast"  placeholder="--Pilih Data--"    class="custom-select" >
                                                        <?php
                                                        $rec = $data['id_petugas_bast'];
                                                        $id_petugas_bast_options = $comp_model -> bast_id_petugas_bast_option_list($data['bulan_bast']);
                                                        if(!empty($id_petugas_bast_options)){
                                                        foreach($id_petugas_bast_options as $option){
                                                        $value = (!empty($option['value']) ? $option['value'] : null);
                                                        $label = (!empty($option['label']) ? $option['label'] : $value);
                                                        $selected = ( $value == $rec ? 'selected' : null );
                                                        ?>
                                            ...
                                            <div class="col-sm-8">
                                                <div class="">
                                                    <select required=""  id="ctrl-rincian_output_bast" data-load-path="<?php print_link('api/json/bast_rincian_output_bast_option_list') ?>" name="rincian_output_bast"  placeholder="--Pilih Data--"    class="custom-select" >
                                                        <?php
                                                        $rec = $data['rincian_output_bast'];
                                                        $rincian_output_bast_options = $comp_model -> bast_rincian_output_bast_option_list($data['id_petugas_bast']);
                                                        if(!empty($rincian_output_bast_options)){
                                                        foreach($rincian_output_bast_options as $option){
                                                        $value = (!empty($option['value']) ? $option['value'] : null);
                                                        $label = (!empty($option['label']) ? $option['label'] : $value);
                                                        $selected = ( $value == $rec ? 'selected' : null );
                                                        ?>
